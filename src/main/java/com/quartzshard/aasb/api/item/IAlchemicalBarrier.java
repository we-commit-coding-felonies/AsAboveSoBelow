...
public interface IAlchemicalBarrier {
	...
	default boolean shieldWithEmc(Player player, float damage, DamageSource source, ItemStack stack) {
        ...
        if (doDebug) {
        	...
        	LogHelper.debug("IAlchemicalBarrier", "ShieldingDebug", "Attempting to shieldWithEmc", info);
        }
        
		if (shieldCondition(player, damage, source, stack)) {
			if (damage <= 0) return true;
			long emcCost = calcShieldingCost(player, damage, source, stack);
			long emcHeld = Long.MAX_VALUE;//EmcHelper.getAvaliableEmc(player);
			if (emcCost <= emcHeld && emcHeld > 0) {
				long emcConsumed = 0;//EmcHelper.consumeAvaliableEmc(player, emcCost);
				if (emcConsumed > emcCost) {
					player.level.playSound(null, player, EffectInit.Sounds.WAY_WASTE.get(), SoundSource.PLAYERS, 0.45F, 1.0F);
				}
				
				emcHeld -= emcConsumed;
				if (emcHeld > 0) {
					player.level.playSound(null, player, EffectInit.Sounds.BARRIER_PROTECT.get(), SoundSource.PLAYERS, 0.45F, 1.0F);
				} else {
