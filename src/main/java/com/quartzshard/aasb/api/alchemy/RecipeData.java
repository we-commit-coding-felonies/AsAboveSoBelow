...
public class RecipeData {
	...
	@Nullable
	public static RecipeData fromRecipe(Recipe<?> recipe, RegistryAccess ra) {
		Tuple<ItemData,Integer> output = new Tuple<>(ItemData.fromStack(recipe.getResultItem(ra)), recipe.getResultItem(ra).getCount());
		...
		if (!ings.isEmpty()) {
			inputs = NonNullList.of(Ingredient.EMPTY, ings.toArray(new Ingredient[0]));
	...
	@Nullable
	public static NonNullList<RecipeData> fromRecipeOld(Recipe<?> recipe, RegistryAccess ra) {
		Tuple<ItemData,Integer> output = new Tuple<>(ItemData.fromStack(recipe.getResultItem(ra)), recipe.getResultItem(ra).getCount());
		...
		for (Ingredient ing : recipe.getIngredients()) if (!ing.isEmpty()) {
			 ...
			 * the reason it isnt used is because it fills all avaliable RAM with variants of recipes with wood in them
