			...
			if (ClientUtil.shiftHeld()) {
				if (runesVal > 1) {
					if (hasRune(stack, AlchInit.RUNE_WATER.get())) {
						...
						tips.add(LangData.tc(LangData.TIP_SHOVEL_FLAVOR).copy().withStyle(ChatFormatting.UNDERLINE));
						tips.add(LangData.tc(LangData.TIP_SHOVEL_DESC, Keybinds.Bind.ITEMFUNC_1.fLoc()));
