...
public class HomingArrowNodeEvaluator extends NodeEvaluator {

	@Nullable public AbstractArrow arrow;
	...
	public int getNeighbors(Node[] neighbours, Node thisNode) {
		...
		if (HomingArrowNodeEvaluator.isOpen(south)) {
			neighbours[i++] = south;
		}
		...
		if (HomingArrowNodeEvaluator.isOpen(east)) {
			neighbours[i++] = east;
		}

		Node north = this.getNode(thisNode.x, thisNode.y, thisNode.z - 1);
		if (HomingArrowNodeEvaluator.isOpen(north)) {
			neighbours[i++] = north;
		}
		...
		if (HomingArrowNodeEvaluator.isOpen(below)) {
			neighbours[i++] = below;
		}
