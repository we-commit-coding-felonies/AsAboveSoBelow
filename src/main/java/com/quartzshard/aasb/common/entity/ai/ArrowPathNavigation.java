...
public class ArrowPathNavigation {
	...
	public ArrowPathNavigation(AbstractArrow arrow, Level level) {
		this.arrow = arrow;
		this.level = level;
	...
	public void tick() {
		...
		if (!isDone()) {
			if (canUpdatePath()) {
				...
				if (tmpPos.y > nextPos.y && !arrow.isOnGround() && Mth.floor(tmpPos.x) == Mth.floor(nextPos.x) && Mth.floor(tmpPos.z) == Mth.floor(nextPos.z)) {
					path.advance();
				}
			}
