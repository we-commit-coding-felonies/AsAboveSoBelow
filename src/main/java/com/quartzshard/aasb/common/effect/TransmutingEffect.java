package com.quartzshard.aasb.common.effect;
...
public class TransmutingEffect extends MobEffect {
	...
	public TransmutingEffect() {
		super(MobEffectCategory.NEUTRAL, Color.PHILOSOPHERS.I);
	}
	
	@Override
	public List<ItemStack> getCurativeItems() {
		ArrayList<ItemStack> cures = new ArrayList<ItemStack>();
		cures.add(new ItemStack(ObjectInit.Items.PHILOSOPHERS_STONE.get()));
		return cures;
	}
			...
			if (amplifier > 0) {
				...
				if (effect != null && effect.getDuration() <= 2) {
					if (entity.getRandom().nextInt(15+amplifier) == 0) {
						...
						if (amplifier > 1) {
							entity.addEffect(new MobEffectInstance(ObjectInit.MobEffects.TRANSMUTING.get(), amplifier, amplifier - 1));
						} else {
							entity.addEffect(new MobEffectInstance(ObjectInit.MobEffects.TRANSMUTING.get(), 300*amplifier, 0));
						}
					}
				}
				return;
